...
namespace SQLiteADOTestSDK
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("Welcome to SQLite uisng ADO.NET, C# on Windows Embedded Compact 2013");
            ...
            for (int i=0;i<2;i++) 
            {
                ...
                //Probably not a good idea to create the db where the dll are as they could be XIP in ROM.
                ...
                while (reader.Read())
                {
                    Console.WriteLine(" {0} = {1}", reader.GetName(0),reader[0]);// .GetInt32(0)); // + " " + reader.GetString(1) );
                }
                reader.Close();
